...
export async function repurposeAnchorContent(anchorTitle, anchorContent, painPoints, solutionSteps, llmClient, configLoader) {
    ...
    const systemPrompt = config.getSystemPrompt("You are repurposing anchor blog content into multiple short-form formats following Jana Osofsky's capsule content strategy.");
  ...
  "pinterest_pins": ["pin 1", "pin 2", "pin 3"]
    ...
    try {
        ...
        if (response.social_posts && Array.isArray(response.social_posts)) {
            response.social_posts.forEach((post) => {
                repurposed.push({
                    type: "social_post",
                    content: post,
        ...
        if (response.pinterest_pins && Array.isArray(response.pinterest_pins)) {
            response.pinterest_pins.forEach((pin) => {
                repurposed.push({
                    type: "pinterest_pin",
                    content: pin,
...
//# sourceMappingURL=repurposer.js.map