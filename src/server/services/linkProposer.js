...
export async function proposeLinksForConcept(conceptId, maxProposals = 5, database, llmClient, configLoader) {
    ...
    const proposals = await proposeLinksWithLLM(targetConcept, candidates, maxProposals, client, config, dbInstance);
...
async function proposeLinksWithLLM(sourceConcept, candidates, maxProposals, llmClient, configLoader, database) {
    ...
    const systemPrompt = configLoader.getSystemPrompt("You are analyzing relationships between concepts in a knowledge graph. Propose meaningful, typed links.");
...
//# sourceMappingURL=linkProposer.js.map